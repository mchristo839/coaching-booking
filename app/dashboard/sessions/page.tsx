    ...
    <div className="mb-6">
      ...
      <div className="space-y-2">
        {sessions.map((s) => {
          ...
          return (
            <div key={s.id} className={`bg-white rounded-xl shadow-sm p-4 ${muted ? 'opacity-75' : ''}`}>
              <div className="flex justify-between items-start mb-2">
                <div>
                  <div className="font-semibold text-gray-900">
                    {s.member_name || 'Unknown member'}
                    {s.member_phone && <span className="text-gray-400 font-normal ml-2 text-xs">{s.member_phone}</span>}
                  </div>
                ...
                <div className="flex flex-col gap-1 items-end">
                  <span className={`text-xs px-2 py-1 rounded ${STATUS_COLOURS[s.status] || 'bg-gray-100'}`}>
                    {s.status.replace('_', ' ')}
                  </span>
                  {s.payment_status && (
                    <span className={`text-xs px-2 py-1 rounded ${PAYMENT_COLOURS[s.payment_status] || 'bg-gray-100'}`}>
