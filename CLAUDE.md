...
- **Two ways to start it (both supported)**:
  ...
  3. **Group enquiry** (Route 2) — when a live camp exists for the group (`getActiveCampForProgramme`), the closed-intent bot **always confirms first**: after answering an in-scope question, or when it detects a `capacity_booking` intent ("how do I book", "can I book"), it asks *"Would you like me to send you the booking link? Reply yes"* and records a `camp_offers` row. A **yes** then calls `startCampBookingFromPoll` and DMs the parent. The booking link is **never posted in the group** — it only ever goes out in the 1:1 DM.
