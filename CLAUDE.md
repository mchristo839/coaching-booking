...
- **Two ways to start it (both supported)**:
  ...
  3. **Group enquiry** (Route 2) — when a live camp exists for the group (`getActiveCampForProgramme`), the bot offers a booking link **only when a parent directly asks to book** (a `capacity_booking` intent — "how do I book", "can I book", "is there space"): it asks *"Would you like me to send you the booking link? Reply yes"* and records a `camp_offers` row; a **yes** then calls `startCampBookingFromPoll` and DMs the parent. It does **not** tack a booking offer onto ordinary answers (price, what to bring, location, etc.) — that read as repetitive/salesy; those are answered as plain customer service. The booking link is **never posted in the group** — it only ever goes out in the 1:1 DM.
