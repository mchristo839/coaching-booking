...
- **Flow** (`conversation_step` on `camp_bookings`): `awaiting_parent_name → awaiting_child_name → awaiting_child_age → awaiting_day_selection → awaiting_checkout_confirm → awaiting_payment → awaiting_coach_confirm → confirmed` (+ `awaiting_waitlist_confirm`, `cancelled`, and two pre-flow primers — `awaiting_dm_start` for a privacy-LID parent we can't DM, and `awaiting_camp_selection` when a group has >1 live camp). The coarse `state` column is kept in sync for the dashboard/confirm queries.
...
- **"Ready to pay now?" gate + remind ladder**: after checkout-confirm the bot asks *"Are you ready to make payment now? (Yes/No)"* (`awaiting_payment_ready`). **Yes** → the existing sign-up→link flow. **No** → *"Shall I remind you in a few days? (Yes/No)"* (`awaiting_remind_consent`); **Yes** sets a 72h reminder (`reminder_due_at`, step `snoozed`), **No** just goes `snoozed` (no chase). The `payment-chase` cron (`runCampChase`) sends the due reminder — *"Ready to confirm + pay? (Yes/No/Amend)"* (`awaiting_reminder_response`): **Yes**→sign-up→link, **Amend**→re-pick days, **No**→`snoozed`. A snoozed parent who messages again re-enters at *"ready to pay now?"*. Payment-trouble messages at the link step route them to the coach. `snoozed`/reminder steps are never auto-expired by cleanup.
- **Sign-up before payment**: after the "ready to pay → Yes", the flow collects parent full name → email → phone (`awaiting_reg_name/email/phone`), then **registers a `members` row per child** (status `trial`, linked to the programme) so the family shows on the Members page, then sends the payment link.
- **Thank-you / payment-received follow-up** is a WhatsApp message (`buildCampPaymentReceived`, itemised) sent the moment the parent replies "done"/paid. It thanks them, says **"[Coach] will confirm your payment in due course. Thanks for your booking!"** and carries a **GDPR notice** (we only use details to manage the booking, never shared, reply STOP to opt out). (Email was dropped — the WhatsApp follow-up is preferred; the parent's email is still collected onto the member record for the coach's records.)
- **Answers camp questions mid-flow**: the 1:1 payment bot fields questions at any step (`answerCampQuestionMidFlow`) drawing on the **same** programme knowledgebase + approved FAQs the group bot uses (`getProgramWithKbById` → `classifyBotIntent`/`planBotResponse`/`phraseAnswer` + `matchActiveFaq`) plus the camp's own day/price/venue details, then re-prompts the current step so the parent carries on where they left off. It only ever fires when the step's own parser fails (or for free-text name steps) AND it can genuinely answer (`looksLikeCampQuestion` is a conservative gate) — so a valid step answer (name, age, "Monday", "yes", "done") is never hijacked.
...
- **Two ways to start it (both supported)**:
  ...
  3. **Group enquiry** (Route 2) — when a live camp exists for the group (`getActiveCampForProgramme`): ANY message that reads as a **booking request** (`looksLikeBookingRequest` in `camp-booking.ts` — "how do I book", "can I book", "booking link", "send me the link", "sign me up", "reserve/register", "book a place", etc.) **directly** calls `startCampBooking(programmeId, jid, name)` and DMs the parent to start the 1:1 booking, then posts a brief group ack (*"Great — I've just sent you a DM to get you booked in 👍"*). It runs **before the question gate** so non-questions ("Booking link") trigger it too, and **ahead of the availability answer** so the bot delivers the link instead of routing to the coach. Ordinary camp questions (price, what to bring, location, which days) are still just **answered** — no booking pitch. The booking link is **never posted in the group** — it only ever goes out in the 1:1 DM.
- **Can't-DM (privacy LID) handling**: WhatsApp's number-privacy feature exposes some group members only by an opaque `…@lid` (no phone number), so the bot can't *open* a DM to them — a send silently goes nowhere. On a Route-2 booking request from a `@lid` sender, `startCampBooking` opens a **primer booking** at step `awaiting_dm_start` (no futile DM) and the group ack instead asks the parent to **message the bot first** (*"sorry, I can't message you directly… send me a quick DM first and I'll get you booked in"* — logged as `booking_request_lid`). Their first 1:1 DM is **adopted by WhatsApp name** (`pickAdoptableLidBooking`, re-keyed to their phone JID) and the `awaiting_dm_start` handler then sends the proper opening — so the conversation continues with full context. *Limitation*: adoption needs a usable `pushName` to match on; if the group `pushName` was junk/empty the booking can't be re-linked (a known `pushName` quirk).
  - **Multiple live camps → parent chooses** (`getActiveCampsForProgramme`, all live holiday_camps for the programme, oldest first): when a group is running more than one camp at once (e.g. a July week and an August week), the booking DM first asks **which camp** (step `awaiting_camp_selection`, `buildCampChoice`/`parseCampChoice` — number, letter, or title/month/date match), then proceeds into the normal name → child → days flow on the chosen camp (`setBookingPromotion`). One camp → straight into the opening as before; the poll-YES and cohort paths still target their specific linked camp. **Deduped** so a parent never sees the same camp twice: `DISTINCT ON (p.id)` (guards duplicate `promotion_targets` rows) **plus** a content signature (title + days) that collapses accidental duplicate promotions — so identical camps fold into one choice (and, if only one distinct camp remains, the choice is skipped entirely). Same-titled-but-different-date camps are disambiguated with a date range in the prompt.

## Clients database (Members page)

`/dashboard/members` is the coach's full client record. Per member it shows parent name, child name, **child age** (from their camp booking, else derived from DOB), **sessions booked/paid** (camp days across their non-cancelled bookings vs confirmed/paid ones), programme, status, joined date and contact details — plus a per-day attendee view on the camp page (`/dashboard/promotions/[id]`). The age + session aggregates are computed in `listMembersByCoach` / `listMembersByProgramme` (a `LEFT JOIN LATERAL` over `camp_bookings`).
