...
- **Two ways to start it (both supported)**:
  ...
  3. **Group enquiry** (Route 2) — when a live camp exists for the group (`getActiveCampForProgramme`): a **direct booking ask** (a `capacity_booking` intent — "how do I book", "can I book", "is there space") makes the bot ask *"Would you like me to send you the booking link? Reply yes"*; a **yes** then calls `startCampBookingFromPoll` and DMs the parent. For ordinary answers (price, what to bring, location, etc.) the bot does **not** pitch on every reply — at most it appends a **single one-time, low-key nudge** to a parent's first camp-relevant answer (*"(Whenever you're ready, I can get you booked in… just let me know 👍)"*), gated by `hasRecentCampOffer` so it never repeats. Both paths record a `camp_offers` row, so a follow-up **yes** books them. The booking link is **never posted in the group** — it only ever goes out in the 1:1 DM.
